...
function serializePublicLink(row: any) { return { id: row.public_id, title: row.title, amount: Number(row.amount), currency: row.currency, description: row.description, expiresAt: row.expires_at, paymentMethods: row.payment_methods, status: row.status === 'ACTIVE' && isExpired(row) ? 'EXPIRED' : row.status, createdAt: row.created_at, updatedAt: row.updated_at }; }
function serializePublicPayment(row: any) { return { id: row.id, provider: row.provider, amount: Number(row.amount), currency: row.currency, status: row.status, createdAt: row.created_at, updatedAt: row.updated_at, completedAt: row.completed_at }; }
...
export function buildApp() {
  ...
  app.register(helmet);
  app.register(cors, { origin: process.env.FRONTEND_URL ? [process.env.FRONTEND_URL] : process.env.NODE_ENV === 'production' ? false : true, credentials: true });
  ...
  app.addHook('preHandler', async (request, reply) => {
    ...
    try {
      ...
      const merchant = await getPool().query('SELECT status FROM merchants WHERE id=$1 LIMIT 1', [auth.merchantId]);
      if (!merchant.rowCount || merchant.rows[0].status !== 'ACTIVE') return reply.code(403).send({ success: false, message: 'Merchant account is inactive' });
    } catch (error) {
  ...
  app.setNotFoundHandler(async (_request, reply) => reply.code(404).send({ success: false, message: 'Route not found' }));
  app.setErrorHandler(async (error, request, reply) => { request.log.error(error); if (reply.sent) return; if ((error as any).statusCode === 413) return reply.code(413).send({ success: false, message: 'Request body is too large' }); return reply.code((error as any).statusCode && (error as any).statusCode >= 400 && (error as any).statusCode < 600 ? (error as any).statusCode : 500).send({ success: false, message: 'Internal server error' }); });
  ...
  app.post('/api/v1/auth/register', { config: { rateLimit: { max: 8, timeWindow: '1 minute' } } }, async (request, reply) => {
  ...
  app.get('/api/v1/payment-links/:id', async (request, reply) => {
    ...
    try { await ensureDatabase(); const result = await getPool().query('SELECT * FROM payment_links WHERE public_id=$1 LIMIT 1', [params.data.id]); if (!result.rowCount) return reply.code(404).send({ success: false, message: 'Payment link not found' }); return reply.send({ success: true, data: serializePublicLink(result.rows[0]) }); }
    catch (error) { request.log.error(error); return reply.code(503).send({ success: false, message: 'Database is unavailable' }); }
  });
  ...
  app.post('/api/v1/payment-links/:id/pay/bkash', { config: { rateLimit: { max: 20, timeWindow: '1 minute' } } }, (request, reply) => initiate(request, reply, 'bkash'));
  app.post('/api/v1/payment-links/:id/pay/nagad', { config: { rateLimit: { max: 20, timeWindow: '1 minute' } } }, (request, reply) => initiate(request, reply, 'nagad'));

  app.get('/api/v1/payments/bkash/callback', async (request, reply) => {
    const query = z.object({ paymentID: z.string().min(1).max(100), status: z.enum(['success','failure','cancel']).optional() }).safeParse(request.query); if (!query.success) return reply.code(400).send({ success: false, message: 'Invalid bKash callback' });
    try { await ensureDatabase(); const db = getPool(); const result = await db.query("SELECT * FROM transactions WHERE provider='bkash' AND provider_payment_id=$1 LIMIT 1", [query.data.paymentID]); if (!result.rowCount) return reply.code(404).send({ success: false, message: 'Transaction not found' }); const tx = result.rows[0]; if (tx.status !== 'SUCCESS') { if (query.data.status === 'cancel' || query.data.status === 'failure') await db.query("UPDATE transactions SET status='FAILED',updated_at=NOW() WHERE id=$1 AND status<>'SUCCESS'", [tx.id]); else { let verified: any; try { verified = await executeBkashPayment(query.data.paymentID); } catch { verified = await queryBkashPayment(query.data.paymentID); } if (verified.transactionStatus === 'Completed' || verified.statusCode === '0000') await db.query("UPDATE transactions SET status='SUCCESS',provider_transaction_id=COALESCE($1,provider_transaction_id),updated_at=NOW(),completed_at=COALESCE(completed_at,NOW()) WHERE id=$2 AND status<>'SUCCESS'", [verified.trxID ?? null, tx.id]); else if (verified.transactionStatus === 'Initiated' || verified.transactionStatus === 'Pending') await db.query("UPDATE transactions SET status='PENDING',updated_at=NOW() WHERE id=$1 AND status<>'SUCCESS'", [tx.id]); else await db.query("UPDATE transactions SET status='FAILED',updated_at=NOW() WHERE id=$1 AND status<>'SUCCESS'", [tx.id]); } } const frontend = process.env.FRONTEND_URL; if (frontend) return reply.redirect(303, `${frontend}/pay/success?transaction=${encodeURIComponent(String(tx.id))}`); return reply.send({ success: true, transactionId: tx.id }); }
  ...
  app.get('/api/v1/payments/nagad/callback', async (request, reply) => {
    const query = z.object({ payment_ref_id: z.string().min(1).max(100), status: z.string().max(50).optional(), status_code: z.string().max(50).optional(), issuer_payment_ref: z.string().max(100).optional() }).safeParse(request.query); if (!query.success) return reply.code(400).send({ success: false, message: 'Invalid Nagad callback' });
    try { await ensureDatabase(); const db = getPool(); const result = await db.query("SELECT * FROM transactions WHERE provider='nagad' AND provider_payment_id=$1 LIMIT 1", [query.data.payment_ref_id]); if (!result.rowCount) return reply.code(404).send({ success: false, message: 'Transaction not found' }); const tx = result.rows[0]; if (tx.status !== 'SUCCESS') { const verified = await verifyNagadPayment(query.data.payment_ref_id); if (isNagadSuccess(verified.status)) await db.query("UPDATE transactions SET status='SUCCESS',provider_transaction_id=COALESCE($1,provider_transaction_id),updated_at=NOW(),completed_at=COALESCE(completed_at,NOW()) WHERE id=$2 AND status<>'SUCCESS'", [verified.issuerPaymentRefNo ?? query.data.issuer_payment_ref ?? null, tx.id]); else if (['OrderInitiated','Ready','InProgress','OtpSent','OtpVerified','PinGiven'].includes(String(verified.status))) await db.query("UPDATE transactions SET status='PENDING',updated_at=NOW() WHERE id=$1 AND status<>'SUCCESS'", [tx.id]); else await db.query("UPDATE transactions SET status='FAILED',updated_at=NOW() WHERE id=$1 AND status<>'SUCCESS'", [tx.id]); } const frontend = process.env.FRONTEND_URL; if (frontend) return reply.redirect(303, `${frontend}/pay/success?transaction=${encodeURIComponent(String(tx.id))}`); return reply.send({ success: true, transactionId: tx.id }); }
  ...
  app.get('/api/v1/payments/:id', async (request, reply) => {
    ...
    try { await ensureDatabase(); const result = await getPool().query('SELECT id,provider,amount,currency,status,created_at,updated_at,completed_at FROM transactions WHERE id=$1 LIMIT 1', [params.data.id]); if (!result.rowCount) return reply.code(404).send({ success: false, message: 'Transaction not found' }); return reply.send({ success: true, data: serializePublicPayment(result.rows[0]) }); } catch (error) { request.log.error(error); return reply.code(503).send({ success: false, message: 'Database is unavailable' }); }
  });
