...
test('readiness endpoint reports unavailable database instead of crashing',async()=>{const r=await app.inject({method:'GET',url:'/health/ready'});assert.ok([200,503].includes(r.statusCode));assert.equal(r.headers['content-type'].includes('application/json'),true)});
