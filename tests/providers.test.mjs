...
test('bKash provider surfaces gateway HTTP failures', async () => {
  ...
  try { const {createBkashPayment}=await import('../dist/providers/bkash.js');await assert.rejects(()=>createBkashPayment({amount:'10.00',invoice:'invoice-2'}),/bKash HTTP 502/); } finally { globalThis.fetch=originalFetch; }
});
